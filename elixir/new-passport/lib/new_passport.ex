defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
         ...
         final_passport_number <- get_new_passport_number(timestamp, counter_number, checksum) do
      {:ok, final_passport_number}
    else
      {:coffee_break, _} ->
        {:retry, now |> NaiveDateTime.add(:timer.minutes(15), :millisecond)}
